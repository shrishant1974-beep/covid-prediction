...
cors_origins = os.environ.get(
    "CORS_ORIGINS",
    "https://shrishant1974-beep.github.io,http://localhost:5000,http://127.0.0.1:5000",
)
...
def get_summary():
    ...
    try:
        ...
        return jsonify({"error": "Unable to load summary data."}), 500
    finally:
        cursor.close()
...
def get_analysis():
    ...
    try:
        ...
        app.logger.exception("Analysis endpoint failed")
        return jsonify({"error": "Unable to load analysis data."}), 500
    finally:
        cursor.close()
...
def get_districts():
    ...
    try:
        ...
        app.logger.exception("Districts endpoint failed")
        return jsonify({"error": "Unable to load district data."}), 500
    finally:
        cursor.close()
...
def get_covid():
    ...
    try:
        ...
        app.logger.exception("Covid endpoint failed")
        return jsonify({"error": "Unable to load covid data."}), 500
    finally:
        cursor.close()
